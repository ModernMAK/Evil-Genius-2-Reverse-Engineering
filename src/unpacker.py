...
#
#
# def unpack_as_json(data, full_path: str = None, *, overwrite: bool = True) -> bool:
#
...
#
#
# def unpack_chunk_as_binary(chunk: BaseChunk, full_path: str = None, *, overwrite: bool = True):
#     with BytesIO() as temp:
#         chunk.write(temp)
#         temp.seek(0)
#         data = temp.read()
#         return unpack_bytes_as_binary(data, full_path, overwrite=overwrite)
#
#
# def unpack_bytes_as_binary(data: bytes, full_path: str = None, *, overwrite: bool = True):
...
#
#
# def unpack_as_meta_and_data(meta: Dict, data: bytes, full_path: str = None, *, overwrite: bool = True):
#
#     success = False  # Im worried about short-circuiting, so the assignment is over two seperate lines
#     success |= unpack_as_json(meta, full_path +".meta", overwrite=overwrite)
#     success |= unpack_bytes_as_binary(data, full_path, overwrite=overwrite)
#     return success
#
#
...
#
#
# # UNPACKED ARCHIVES LAYOUT
# #   ARCHIVE FOLDER (eg name.asr\)
# #       META (eg name.asr\meta.json) ~ Information needed to reconstruct the archive
# #       CHUNK FOLDERS (eg name.asr\chunk {id}.{type}\)
# #           META (eg name.asr\meta.json) ~ Information needed to reconstruct the chunk
# #           resource/asts formats:
# #              RESOURCES (eg name.asr\chunk {id}.{type}\resources)
# #                  DATA (eg name.asr\chunk {id}.{type}\resources\{file}
# #
#
# # name may not be used for all unpacked assets
# def unpack_chunk(chunk: BaseChunk, archive_name: str, index: int, out_dir: str = None, *,
#                  overwrite: bool = True) -> bool:
#     chunk_name = join(archive_name, f"Chunk {index}.{chunk.header}")
#
#
...
#         archive_name += f".{chunk.language.code}.{}"
...
#
#
# def unpack_archive(archive: BaseArchive, name: str, out_dir: str = None, archive_name: str = None,
